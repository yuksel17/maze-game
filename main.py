...
COLOR_ELIXIR, COLOR_TEXT = (34, 197, 94), (30, 41, 59)
...
def init_game():
    ...
    m.items = {}
    # Proje dokümanındaki 3 ana eşya [cite: 2, 23]
    ...
    return m, list(m.start_pos), [], [], 0, "Manuel", False, 80

def main():
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("Cadth Ajani: Intelligent Solver")
    ...
    while True:
        ...
        screen.blit(font.render(f"Toplanan İksirler: {len(coll)}/3", True, COLOR_ELIXIR), (20, 45))
        screen.blit(font.render("UYARI: Kırmızı tuzaklardan kaçının!", True, COLOR_TRAP), (20, 70))

        for ev in pygame.event.get():
            if ev.type == pygame.QUIT: pygame.quit(); sys.exit()
            if ev.type == pygame.KEYDOWN:
                if ev.key == pygame.K_r: maze, pos, coll, path, used, alg, over, limit = init_game()
                if not over:
                    if ev.key in [pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4]:
                        ...
                        if node: 
                            path = node.path()[1:]; p_idx = 0; limit = used + len(path) + 10
                    ...
                    if mv:
                        if 0<=pos[0]<COLS and 0<=pos[1]<ROWS and maze.grid[pos[1]][pos[0]]==0:
                            ...
                            if pt in maze.wrong_items: over = True # [cite: 35]
        ...
        if used >= limit and not over: over = True
        ...
        for pt, name in maze.items.items():
            if name not in coll: pygame.draw.circle(screen, COLOR_ELIXIR, (pt[0]*CELL_SIZE+20, pt[1]*CELL_SIZE+20+PANEL_H), 10)
        for tp in maze.wrong_items: pygame.draw.circle(screen, COLOR_TRAP, (tp[0]*CELL_SIZE+20, tp[1]*CELL_SIZE+20+PANEL_H), 12)
        ...
        if over:
            txt = "ZAFER!" if len(coll)==3 and tuple(pos)==maze.exit_pos else "BASARISIZ!"
            screen.blit(font.render(txt, True, COLOR_TRAP if "BASARISIZ" in txt else COLOR_ELIXIR), (WIDTH//2 - 40, 90))
